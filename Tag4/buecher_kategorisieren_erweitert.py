...
# benötigst.
# Bücherliste: Python Grundlagen:Programmieren; Einführung in SQL:Datenbanken; Der Pragmatische Programmierer:Entwicklung; Algorithmen Einführung:Computerwissenschaft; Moby Dick:Roman; Einführung in Python:Sachbuch; Krieg und Frieden:Roman; Python Kochbuch:Sachbuch
...
while not eingabe_fertig:
    buch_eingabe = input(
        "Bitte eine neue Buch eintragen in Format \"Titel:kategorie\"\nWenn Sie mehrerer Bücher gleichzeitig anlegen wollen, bitte mit \";\" abtrennen\nWenn Sie fertig sind, bitte \"x\" eingeben\n")
    if buch_eingabe == "x":
        eingabe_fertig = True
    else:
        if buch_eingabe.count(";") > 0:
            buecher = buch_eingabe.split(";")

            for buch in buecher:
                split_buch = buch.split(":")
                liste_buecher.append(
                    [split_buch[0].strip(), split_buch[1].strip()])
        else:
            buch = buch_eingabe.split(":")
            liste_buecher.append([buch[0].strip(), buch[1].strip()])
